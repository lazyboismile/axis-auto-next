...
const Join: NextPage = () => {
	...
	const [input, setInput] = useState({ nick: '', password: '', email: '', type: 'USER' });
	...
	const doSignUp = useCallback(async () => {
		console.warn(input);
		try {
			await signUp(input.nick, input.password, input.email, input.type);
			await router.push(`${router.query.referrer ?? '/'}`);
		} catch (err: any) {
			await sweetMixinErrorAlert(err.message);
		}
						...
						<Stack className={'left'}>
							...
							<Box className={'register'}>
								...
								{loginView ? (
									...
									<Button
										variant="contained"
										disabled={input.nick == '' || input.password == '' || input.email == '' || input.type == ''}
										onClick={doSignUp}
										endIcon={<img src="/img/icons/rightup.svg" alt="" />}
									>
										SIGN UP
